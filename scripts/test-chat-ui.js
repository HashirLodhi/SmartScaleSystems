...
const checks = [
  ...
  ['contextual suggestions are rendered from API responses', /appendSuggestions\(data\.suggestions\)/],
  ['safe action links are rendered from API responses', /appendActions\(data\.actions\)/],
  ['action links are validated before rendering', /!isSafeHref\(action\.href\)/],
